...
date_match = (
    "([0-9]{2}[/-]){2}[0-9]{4}"  # matches dates like '01/15/2018' or '09-07-2016'
)
...
def timestamp_to_epoch_time(timestamp, date="today", tz=None, verbose=True):
    ...
    if timestamp == "now":
        ...
        if verbose:
            print(
                "'timestamp_to_unix_time' revieved a time.struct_time object. "
                + "Returning the corresponding epoch time."
            )
    elif type(timestamp) is not str:
        if verbose:
            print(
                "timestamp_to_unix_time' didn't receive a string. Returning"
                + " the argument."
            )
        return timestamp
    if len(timestamp) > 8 and date == "today":
        if verbose:
            print(
                "'timestamp_to_unix_time' is assuming"
        ...
        try:
            ...
            if verbose:
                print(
                    "timestamp went straight into time.strptime()! Returning based on that."
                )
            return tstamp
        except ValueError:
            if verbose:
            ...
            try:
                ...
                if verbose:
                    print(
                        "I got no clue what you're talking about, dude, "
                        + "when you say "
                        + timestamp
                        + ". It didn't match \."
                        + timestamp_match
...
def load_from_csv(filepath, multiset=False, tstamp=None, verbose=True):
    ...
    data = get_empty_set(colheaders, title=filepath, tstamp=tstamp, data_type="SPEC")
    datasets = []
        ...
        if nl < N_head - 1:  # we're in the header
            ...
            if data_type == "EC":
                ...
                if timestamp is None and a is not None:
                    ...
                    if d is not None:
                        date = d.group()
                    tstamp = timestamp_to_epoch_time(
                        l, tz=tz, verbose=verbose
                    )  # the XAS data is saved with time.ctime()
                    if verbose:
                        print("timestamp '" + timestamp + "' found in line " + str(nl))
                header_string = header_string + line
            ...
            dataset[
                "data_cols"
            ] = col_headers.copy()  # will store names of columns containing data
            ...
            if not len(line_data) == len(col_headers):
                if verbose:
                    ...
                    print(
                        "Mismatch between col_headers and data on line "
                        + str(nl)
                        + " of "
                        + title
                    )
                if (
                    nl == N_lines - 1 and data_type == "MS"
                ):  # this is usually not useful!
                    print(
                        "mismatch due to an incomplete last line of "
                        + title
                        + ". I will discard the last line."
                    )
                    break
                    ...
                    try:
                        ...
                        try:
                            ...
                            if len(dataset["data_cols"]) == 0:
                                if verbose:
                                    print(
                                        "there's a full line of value errors. Must me something wrong. "
                                        + "Terminating loop."
                                    )
...
def load_EC_set(
    directory, EC_file=None, tag="01", verbose=True, tz=None, force_sort=False
):
    if verbose:
    ...
    for f in EC_file:
        try:
            EC_datas += [
                load_from_file(
                    directory + os.sep + f, data_type="EC", tz=tz, verbose=verbose
                )
            ]
        except OSError:
