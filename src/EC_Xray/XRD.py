...
def get_peak_background(x, y, xspan=None, background='linear', 
                   background_points=3, mode='full',
        ...
        if mode == 'match': # return a background equal to the input outside the peak interval
            background = np.append(np.append(y[0:i_start], background), y[i_finish:])
        elif mode == 'full':
            background = ((x - xspan[0]) * y_postpeak + 
                      (xspan[-1] - x) * y_prepeak) / (xspan[-1] - xspan[0])
    ...
    def fit_gauss(self, center=None, sigma=None, ax=None):
        ...
        if center is not None and sigma is not None:
            def gauss_i(x, height):
                return gauss(x, center=center, sigma=sigma, 
                             height=height)
            guess = guess_h
...
def refraction_correction(alpha=0.15, delta_eff=5.94e-6, 
                          beta_eff=2.37e-7, alpha_c=0.197, tth=None):
    ...
    delta_tTh = Alpha - 1/np.sqrt(2) * np.sqrt(
                np.sqrt( (Alpha**2 - Alpha_c**2 + Alpha**2*Alpha_c**2/2)**2
                        + (-2*Beta + Alpha**2*Beta)**2
                        )
            - Alpha_c**2 + Alpha**2*Alpha_c**2/2 + Alpha**2
            )
