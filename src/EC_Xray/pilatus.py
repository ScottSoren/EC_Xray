...
calibration_0 = {
    "direct_beam_x": 255,  # November and May
...
class Pilatus:
    def __init__(
        ...
        if self.calibration is not None:
            self.db_pixel = (
                calibration["direct_beam_y"],  # First dimension is in
    ...
    def get_map_sphere(self, tth=None, override=False):
     ...
     z' ^       (i,j)       Illustrated: transformation of cartesian to spherical coordinates 
        ...
        map_phi = (
            np.arcsin(map_x_p / (np.sqrt(map_x_p ** 2 + map_y_p ** 2))) * 180 / np.pi
        )  # in degrees
    ...
    def tth_spectrum(
        ...
        if (
            not override
            and tth is None
            and "stepsize" in dir(self)
            and "method" in dir(self)
        ):
            if (
                out == "bins"
                and "bins" in dir(self)
                and self.stepsize == stepsize
                and self.method == method
            ):
                return self.bins
            elif (
                out == "spectrum"
                and "spectrum" in dir(self)
                and self.stepsize == stepsize
                and self.method == method
            ):
        ...
        if weight is None:
            ...
            sigma = (
                np.max(np.abs(ys)) / 3
            )  # such that the furthest pixel is weighted at ~1% of the db pixel
