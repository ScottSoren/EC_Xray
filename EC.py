...
I would like for this module to be common between EC_MS and EC_Xray. Just like
the EU and UK regulatory markets, though, I'm likely to see divergence without
active coordination.
I should look into git submodules:
https://stackoverflow.com/questions/1540879/how-to-share-files-across-git-repositories

This means that if there is any relative import, that it must be of corresponding
and identically named functions from an identically named module in each.

Getting this to work will be left for later
        ...
        for col in EC_data['data_cols']:
            ...
            if not is_EC_data(col): #then we've got a QMS or synchrotron variable
                ...
                if time_col in time_masks:
                    ...
                    mask = np.logical_and(tspan[0] < EC_data[time_col] and EC_data[time_col] < tspan[-1])   
                    # I want to do this vectorized from now on!
                    time_masks[time_col] = mask
                if not is_time(col):
                    EC_data[col] = EC_data[col][mask]     
        for time_col, mask in time_masks.items():
            EC_data[time_col] = EC_data[time_col][mask]
